# analyze_poem  для  gpt_poroshki        ЦЕНТРАЛЬНЫЙ БЛОК        АНАЛИЗ
import re 
from accentize_text import accentize_text, lookup_rings
from clean import clean_lines_with_log, ALLOWED_CHARS
...
SCHEME = ("-+-+-+-+-","-+-+-+-+","-+-+-+-+-","-+")  # знак "_" отмечает строку, которую не надо проверять на ударение
     # схема по рифмам
...
def analyze_poem(text: str, scheme: tuple[str]=SCHEME) -> tuple:  
    ...
    # ПРОВЕРКА РИФМЫ      
    log_norhyme, log_rhyme = check_rhymes(clean_poem, SCHEME, RHYMES)
    print(clean_poem)
    print(log_norhyme)
    print(log_rhyme) 
    ...
    # ПОДГОТОВКА К ВЫВОДУ
    ok = True
    #log_final = [log_rhyme]      #log_norhyme        отладка 
