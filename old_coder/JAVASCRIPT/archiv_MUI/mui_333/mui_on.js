...
function MUI (){	
	...
	var arrShow = document.getElementsByName("type_show"),  flagSimpleShow;
	for (var i=0; i < arrShow.length; i++) {
		if(arrShow[i].checked) { flagSimpleShow = arrShow[i].value;};
	};
	...
	if (flagSimpleShow==0){
		...
		document.getElementById("textsmall").style.display = "inherit";
		...
		var tmpWays;
		tmpWays = (flagSimpleShow == 1)? ways: waysShort;
		var tmpFin = fin;
		if (tmpWays[tmpFin]){ // если fin найден 
			while (tmpWays[tmpFin]) {
				shortTS.innerHTML += ("<br>" + tmpWays[tmpFin]); // с повторами
				tmpFin += "."
			}
		}
		else {
			if (shortTS.innerHTML[shortTS.innerHTML.length-1] != "-") shortTS.innerHTML += "<br>" + "-";
			else shortTS.innerHTML += "-";
		}
		
		if(document.getElementById("clear0").checked) { // скрытие лишних панелей
			document.getElementById("textbigout").style.display = "none";
			document.getElementById("textup").style.display = "none"; // ???  др результ в резюме
