...
var arrMemoryRez = []; ////////////////  ПАМЯТЬ   ???  есть ли выгода во времени. изменить?  +история? 
var arrMemoryIdentific = []; ////
...
function MUI (){	
	...
	//// ЮЗЕР  вывод 0 полностью, 1 цепью, 2 цепью упрощ    
	var arrShow = document.getElementsByName("type_show"),  flagShow = 1, flagSimpleShow;
	...
	out: for (var i=1; i < stopLewels; i++) {  ///
		lewel[i] = [];
		if (!lewel[i-1].length || (arrMemoryIdentific[arrMemoryIdentific.length-1] == identific))	break; // если слова кончились - обрыв цикла. не пустой ли lewel     // прервать если полный повтор параметров 
	...
	/////  ПОЛНЫЙ, УПРОЩЁННЫЙ  ИЛИ ПРОСТОЙ ВЫВОД ДАЛЕЕ /////	
	var shortTS = document.getElementById("textsimple");
	
	if (arrMemoryIdentific[arrMemoryIdentific.length-1] != identific) {
		///// полный /////
	...
	if (!flagShow){
		document.getElementById("textup").style.display = "none"; //// скрытие лишних панелей 
		...
		if (arrMemoryIdentific[arrMemoryIdentific.length-1] != identific) dispBig (arrRez, fin, infoMaxMin);  ///////////////////////
	}
	/////  короткий или совсем короткий вывод. ///// 
	var tmpFin = fin;
	var tmpString0 = "", tmpString1 = "";
	if (waysShort[tmpFin]){ // если fin найден 
		tmpFin = fin;
		while (ways[tmpFin]) {
			tmpString0 += ("<br>" + ways[tmpFin]); 
			tmpFin += ".";
		}
		tmpFin = fin;
		while (waysShort[tmpFin]) {
			tmpString1 += ("<br>" + waysShort[tmpFin]); 
			tmpFin += ".";
		}
	}
	else {
		tmpString0 = tmpString1 = "-";
	}
	if (flagSimpleShow == 2 && arrMemoryTS[arrMemoryTS.length-1][0] != tmpString0) shortTS.innerHTML += tmpString0 + "<br>";
	else if (flagSimpleShow == 1 && arrMemoryTS[arrMemoryTS.length-1][1] != tmpString1) shortTS.innerHTML += tmpString1 + "<br>";
	
	arrMemoryTS.push([tmpString0, tmpString1]);
	
	
	
	
	arrMemoryIdentific.push(identific); // запомнить параметры
	return arrRez;   // ?? 
}
...
//[" "    numAll,  i,  word,  j,   partWord0, partWord1, partWord2,     fncName,     k, m,  act     " "]
	...
	if (arrMemoryRez[identific]) {                 /////////////////// ИЗ ПАМЯТИ     
		textRez = arrMemoryRez[identific]
	}
	else arrMemoryRez[identific] = textRez;        /////////////////// В ПАМЯТЬ     
...
// -----------------------------------------------------------------
	...
	var arrSavePreClick = []; // массивы врем хранения подсветки кликом
	...
	elemThisRow.ondblclick = function illuminatDblClick(e) {
		...
		while (row){
			...
			ser = ser.replace(/id=\"\w+\"/,"id='"+s+"'"); // id заменяем       document.getElementById("textsmall").innerHTML
		...
		var tmpTS = document.getElementById("textsmall").innerHTML;
		tmpTS = tmpTS.slice(0,-8);
		document.getElementById("textsmall").innerHTML = tmpTS + "<tr height='8'>" + "</tr>"+ arrRezSmall.join("") + "</table>";
		//alert (document.getElementById("textsmall").innerHTML)
	};
...
function openProper3(){      
	var menu03 = document.getElementById("menu03"); 
	if (menu03.style.top == "-400px") {menu03.style.top ="-1500px"}
	else {menu03.style.top = "-400px";}
...
function openAllProper0(){
	if(!flagProper0){
		...
		document.getElementById("menu03").style.top = "-400px"; 
		flagProper0 = 1;
		flagProper1 = 1;
		flagProper2 = 1;
		flagProper3 = 1;
...
function clearTextSimple (){
	document.getElementById("textsimple").innerHTML = ""; 
	arrMemoryTS.length -= 1;	
}
