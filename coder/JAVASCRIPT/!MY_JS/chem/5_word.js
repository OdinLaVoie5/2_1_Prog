...
function ShowMessage(Feedback){                   
	...
	FDiv.style.top = TopSettingWithScrollOffset(30) + 'px';         // my        30% от главного окнв
...
function HideFeedback(){
	...
	if (Finished == true){
		Finish();
		               		           // my   действия при победе  ??? alert (0x0300);   
	}
}
...
// my   размеры всплыв окна  ???  или нет?
...
function GetPageXY(El) {        			                             // my         ??? не понял! её никто не вызывает
	var XY = {x: 0, y: 0};
	while(El){
		XY.x += El.offsetLeft;
		XY.y += El.offsetTop;
...
function GetScrollTop(){     		                              // my        позиция документа при скролинге
...
function GetViewportHeight(){		                                   // my        высота главн окна
...
function TopSettingWithScrollOffset(TopPercent){		                          // my         учет и размера и скролинга
...
function WriteToInstructions(Feedback) {      // my        в сплыв окне строка интрукции
...
// my    ??    перехват нажатой кнопки ()кнопка назад в браузе
...
// my   не вызывается.  сужается массив до заданного размера случайным выбросом элементов  
...
//----------------------------------------------------------------------------------------
// Перетасовка       my    перенос данных во временный массив Temp 
function Shuffle(InArray){
	// !!!    создать случайную перестановку массива с помощью sort
	InArray.sort(function comp(a, b) {return Math.random()-.5;});
                            
	/*var Num;    // заменил более простым методом перетасовки
	var Temp = new Array();
	...
	for (i=0; i<Len; i++){
		...
		j--;               // my     из временного массива вырезается выпавший случайно элемент  
                           // (вернее вышележащие в массиве ставят на1 вниз)  и помещаем в исходный массив  
	}*/
	return InArray;
}
...
// my     невызывается.     замена проблемного знака  "  в строке
function EscapeDoubleQuotes(InString){        
	return InString.replace(/"/g, '&quot;')
}

function TrimString(InString){         // my        обрезка пробелов в конце, в начале, двойные пробелы меж словами
        var x = 0;
...
function FindLongest(InArray){            // my     невызывается.    нахождение самого длинного эл-та в массиве
...
// my        не вызывается.       определение типа символов
...
// my        не вызывается.     очистка вводов пользовотеля
...
//EXTENSION TO ARRAY OBJECT
 // my        зачем-то переопределяется метод массива  indexOf ?
...
function RemoveBottomNavBarForIE(){        // my        для IE.
...
// my        + отметка времени начала и завершения?       Если есть форма, заполните ее и представьте её.
...
function Card(ID, OverlapTolerance){             // my        ??? 
...
//Need a bugfix for Opera focus problem here
	if (C.opera){FocusAButton();}          //  my фокус после переноса for Opera
	return true;
} 
...
// my        многие переменные здесь не используются

var Punctuation = '';           /////////////////////////////////////////

var Openers = '';
var CorrectResponse = 'Верно!';
var IncorrectResponse = 'Извините! Попробуйте еще раз.';
var ThisMuchCorrect = 'Большая часть Вашего ответа правильна:';
var TheseAnswersToo = 'Эти ответы также правильны:';
var YourScoreIs = 'Ваш счет ';
var NextCorrect = 'Подсказка:';

var FeedbackWidth = 200; //default
var ExBGColor = '#000673';                       // FFFFFF
var PageBGColor = '#000000';                  // C0C0C0
var TextColor = '#FFFFFF';                       //000000
var TitleColor = '#673000';

var DropTotal = 3; // number of lines that will be available for dropping on      my   число линий, которые будут доступны 
var Gap = 4; //Gap between two segments when they're next to each other on a line        my  Промежуток между двумя сегментами, когда они друг рядом с другом на линии
...
function CapFirst(InString){
	...
	if ((Openers.indexOf(InString.charAt(i))>-1)||(InString.charAt(i) == ' ')){// почему 2 раза?
		i++;
	}
...
function CompDrags(a,b){             //   my     ф-ция сортировки
...
// my        процент правильных ответов
		...
		if (TimeOver == true){          // my        если кончилось время
...
var baseWords = ["водород", "гелий", "литий", "бериллий", "бор", "углерод", "азот", "кислород", "фтор", "неон", "натрий", "магний", "алюминий", "кремний", "фосфор", "сера", "хлор", "аргон", "калий", "кальций", "скандий", "титан", "ванадий", "хром", "марганец", "железо", "кобальт", "никель", "медь", "цинк", "галлий", "германий", "мышьяк", "селен", "бром", "криптон", "рубидий", "стронций", "иттрий", "цирконий", "ниобий", "молибден", "технеций", "рутений", "родий", "палладий", "серебро", "кадмий", "индий", "олово", "сурьма", "теллур", "иод", "ксенон", "цезий", "барий", "лантан", "церий", "празеодим", "неодим", "прометий", "самарий", "европий", "гадолиний", "тербий", "диспрозий", "гольмий", "эрбий", "тулий", "иттербий", "лютеций", "гафний", "тантал", "вольфрам", "рений", "осмий", "иридий", "платина", "золото", "ртуть", "таллий", "свинец", "висмут", "полоний", "астат", "радон", "франций", "радий", "актиний", "торий", "протактиний", "уран", "нептуний", "плутоний", "америций", "кюрий", "берклий", "калифорний", "эйнштейний", "фермий", "менделевий", "нобелий", "лоуренсий", "резерфордий", "дубний", "сиборгий", "борий", "хассий", "мейтнерий"];

//  ???  дублировать базу слов  и удалять из дубля пройденные слова
 

var randomЕlement = Math.round(Math.random()*(109) -0.5);
...
//  !!!   нумерация с проверкой на повторы          ???  сделать ф-цию в osc (проверка на повторы ...)
var addMy = 0;

outer : for (var i=0; i<baseWords[randomЕlement].length; i++) {
  for (var j=0; j<i ; j++) {
	  if (baseWords[randomЕlement][i] == baseWords[randomЕlement][j]) {
		  Answers[0].push(j+1);  
		  addMy++; 
		  continue outer;
	  }
  }
  Answers[0].push(i+1-addMy)
}


var Segments = [];
//   !!   делаем ряд объектов (здесь массивов)  циклом
for (var i=0; i<baseWords[randomЕlement].length; i++) {
  Segments[i] = new Array();
  Segments[i][0] = baseWords[randomЕlement][i];
  Segments[i][1] = Answers[0][i];
  Segments[i][2] = 0;
}
...
function doDrag(e) {                          //    ??? почему дубль ф-ций?
	if (CurrDrag == -1) {return};
	if (C.ie){var Ev = window.event}else{var Ev = e}
	var difX = Ev.clientX-window.lastX; 
	var difY = Ev.clientY-window.lastY; 
...
// my   если на линии две карты накладываются;
function CheckOver(NoMove){
//This recursive function spreads out the Cards on a line if two of them are overlapping;      
//if the spread operation moves one beyond the end of a line, it wraps it to the next line.              к следующей строке. 
...
//Fix for 6.2.5.2 image dragging bug
		...
		Cds[i].css.backgroundColor = '#FFFFFF';          // my        цвет карточки
...
function TimerStartUp(){                                       //  my  таймер
...
//  my таймер в завис от длины слова.
//CODE FOR HANDLING TIMER
//Timer code
var Seconds = ~~(60*Answers[0].length*Answers[0].length/16-15);                 
////////////////////////////////////////////            
var Interval = null;

function StartTimer(){
	Interval = window.setInterval('DownTime()',1000);   //  my  часы тикают по 1 сек
