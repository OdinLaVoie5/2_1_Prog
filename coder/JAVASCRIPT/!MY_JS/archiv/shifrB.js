//+ см   кодир-декодир  
//  вариант   база-алфавит с ё Ё   ... ...     107         
//     учесть  ...       + регул выраж ???  
...
// ?? англ  буквы ...  сделать кнопку перемены базы.  или   определять тип базы при вводе ?? + индикатор типа.
var base = "абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ0123456789_+-,.;:?!'=`~@#$%^&*(){}[]|<> ¶"; // база по умолчанию.  \/ убрал т.к. в degrons проблемы (регул выраж?)  ?? 
var base1 = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"; 
...
//  ???  при втором нажатии др вид (ещё флаг?)     ??  учесть перенос строки
...
var rez = [], temp;
  for (var i=0; i<val.length; i++) {
	temp = base.indexOf(val[i]);
/*	document.write(kluchB.toString(2));    document.write("<br>");
	document.write(temp.toString(2));    document.write("<br>");
*/	
	temp^=kluchB
/*	document.write(temp.toString(2));    document.write("<br>");
*/	
	...
	if(temp>106){  document.write(i+' /');}	
/*	document.write(rez[i]);    document.write("///<br>");
*/  }
  return rez;
}
