...
// -----------------------------------------------------------------
	...
	for (var i = 0; i < infoMaxMin["maxNumAll"]; i++) { 
		...
		if (tmpRez){  // число повторов (точек)  столбец
			...
			elemThisRow.cells[7].innerHTML = "<span style='background-color:rgb(0,250,0)'>"+(tmpRez+1)+"</span>"; // подсветка
			var elemPreThisRow = document.getElementById(elemThisRow.cells[5].innerHTML); // предыдущ ряд
			elemPreThisRow.cells[7].innerHTML = "<span style='background-color:rgb(0,250,0)'>" +1+"</span>";
		}; 
