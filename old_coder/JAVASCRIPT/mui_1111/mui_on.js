...
// ??? проблема -   textsmallout (Выделенные цепочки)  некорект (при повторе?) - упростить.
...
function MUI (){	
	...
	var alfabet = "MUI_" // алфавит ???  вирт клава ...
	
	var start = "", fin = ""; //??? если какой-нибудь эл-т нулевой(какой id сделать?) "" или " "
	start = document.getElementById("startname").value;   //  ЮЗЕР 
	fin   = document.getElementById("finname").value; //  ЮЗЕР  
	// убрать пробелы
	if (~start.indexOf(" ")) {alert ("В слове не должно быть пробелов"); start = start.replace(/\s+/g,""); stupid++ };
	if (~fin.indexOf(" ")) {alert ("В слове не должно быть пробелов"); fin = fin.replace(/\s+/g,""); stupid++ };
	if (stupid == 5) {alert ("Можно имитировать пробел знаком '_'"); stupid = 0};
	document.getElementById("startname").value = start;	
	document.getElementById("finname").value = fin;
	/// нулевой ввод
	if (start=="") {start = "[zero]";}
	if (fin=="") {fin = "[zero]";}
	...
	if (flagRepeatAll && flagRepeatLewel) stopLewels = 25;   //  повторы и так и так.
	...
	displaySimple (arrRez, fin, infoMaxMin)  // простой вывод
	if (identific != memoryIdentific) {displayBig (arrRez, fin, infoMaxMin)}  // вывод полного рез если не повтор
	else setFncsColor (infoMaxMin); // функция стилей цвета ф-ций


	memoryIdentific = identific; // запомнить параметры 
	return arrRez;   // ?? не использую пока. 
}
...
function displaySimple (arrRez, fin, infoMaxMin){
	...
	setMenuHeight (); // оформление панелей меню 
	...
	var txtSimple  = document.getElementById("textsimple");
	...
	if (tmpWays[tmpFin]){ // если fin найден 
		...
		if (arrMemoryTS[arrMemoryTS.length - 1]!= tmpString) {// избегать повторов с последним выводом.   пришлось глобаль перем. 
			txtSimple.innerHTML += tmpString + "<br>";  
		} 
	}
	else {
		tmpString = "-";
		txtSimple.innerHTML += tmpString;
	}
...
function  displayBig(arrRez, fin, infoMaxMin){
	...
	if (arrRez[0]) {
		separ = alignTextOSC ( ("<td>"+" "+ "</td>"),arrRez[0].length-3, arrIgnor.length); // ??
	}
	...
	if (!infoMaxMin["numYes"].length) {		
		document.getElementById("textup11").innerHTML= "&nbsp;&nbsp;" + "-"  + "."   + "-" + ". "  ; // резюме верхнее	 +дописать входн и выходн данные
	}
	else {
	...
	var tempVar1 = (infoMaxMin["maxNumAll"])? (infoMaxMin["maxNumAll"]):"-";
	var tempVar2 = (infoMaxMin["maxI"])? (infoMaxMin["maxI"]):"-";
	document.getElementById("textup21").innerHTML = "&nbsp;&nbsp;" + tempVar1 + "." + tempVar2 + ". ";	
	...
	setFncsColor (infoMaxMin); // функция стилей цвета ф-ций
...
function  illuminatChain (arrRez, fin, infoMaxMin){
	...
	document.getElementById("textsmall").innerHTML = "<table>"  + "</table>";
	...
	for (var i = 0; i < infoMaxMin["maxNumAll"]; i++) { 
		...
		elemThisRow.ondblclick = function illuminatDblClick(e) {
			...
			if (this.id == fin) {// подсветка результата
				elemFin.firstChild.style.backgroundColor = colorFin; 
				memoryRezSmall = "<table>" + memoryRezSmallPart+ "</table>"; 
	...
	// ##jq  ##trigger  ##dblclick 
	if (elemFin) {
		$(elemFin).trigger("dblclick"); // имитирует двойной клик !!!
		// $(elemFin).trigger("mouseover"); // имитирует наведение мыши 
		// $(elemFin).trigger("click"); // имитирует клик 
	}else {memoryRezSmall = "<table>" + "</table>";} 
}
...
function setMenuHeight (){
    ...
    startname.onkeypress = nnn;
    finname.onkeypress = nnn;
function nnn(e){ 
	e = e||window.event;   // ##событие !!!
	if (e.keyCode === 13) {
		butcheck.click();	
...
function setFncsColor (infoMaxMin){ ///  111
	// ЮЗЕР  создание массива цветов ф-ций      тип ф-ции  св-ва   ?????  
	var arrFncColor = []; 
	for (var i=0; i < infoMaxMin["arrFnc"].length; i++) {
		if((typecolor_0.children[i].value != "#000000") && (typecolor_0.children[i].value != "")) {
			arrFncColor[i] = typecolor_0.children[i].value;
			continue;
		};
		if (!infoMaxMin["arrFnc"][i].type){ // нет типа.
			arrFncColor[i] = "rgb(210,210,210)";
			continue;
		};
		arrFncColor[i] = "Goldenrod" // тип не определён.   цвет по умолчанию.
		if(~infoMaxMin["arrFnc"][i].type.indexOf("--")) {arrFncColor[i] = "rgb(255,0,0)"};
		if(~infoMaxMin["arrFnc"][i].type.indexOf("-+")) {arrFncColor[i] = "rgb(255,0,255)"};
		if(~infoMaxMin["arrFnc"][i].type.indexOf("++")) {arrFncColor[i] = "rgb(0,255,255)"};
		if(~infoMaxMin["arrFnc"][i].type.indexOf("**")) {arrFncColor[i] = "rgb(0,255,0)"};
	};

	/// создаём цветовые стили по именам ф-ций///
	for (var i=0; i < infoMaxMin["arrFnc"].length; i++) {
		var tmpStyle = document.getElementsByClassName(infoMaxMin["arrFnc"][i].name+"_"); 
		for (var s=0; s < tmpStyle.length; s++) {
			tmpStyle[s].style.color = arrFncColor[i];
		}; 
	};
}
