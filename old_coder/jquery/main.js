 ...
 *     notificationUsers: Array.<Internal.NotificationUser>
 * }}
 */
Internal.Message;
...
Internal.Server = function() {
    ...
    /**
     * Notifies host that an attachment in a notification message has been
     * clicked.
     *
     * @param {string} id Attachment ID
     * @param {string} type either 'notebook' or 'note'
     * @param {Node} sourceNode The bounding box of this node will be sent.
     *
     * @suppress {undefinedVars|missingProperties}
     */
    me.openNotificationAttachment = function(id, type, sourceNode) {
        var bbox = sourceNode.getBoundingClientRect();
        var top = bbox.top; // Note: relative to viewport, not top of page!
        var left = bbox.left;
        var width = bbox.width;
        var height = bbox.height;

        console.log('To Server: open notification attachment.',
                    'id=' + id, 'type=' + type, 'top=' + top, 'left=' + left,
                    'width=' + width, 'height=' + height, 'and the node',
                    sourceNode);

        try {
            Evernote.openNotificationAttachment(id, type, top, left,
                                                width, height);
        } catch (err) {
            console.warn('Couldn\'t run native function:', err.message);
        }
...
Internal.Runtime = function() {
    ...
    /**
     * Sets a global language flag so that CSS can dynamically change (fonts).
     */
    me.setLanguage = function() {
        var lang = 'en';

        if (window['Evernote'] != null && window['Evernote']['getLanguage'] != null) {
            lang = Evernote['getLanguage']();
        } else {
            lang = navigator.language || navigator['userLanguage'] || 'en';
            // TODO:athorp:2015-04-19 There's probably more edge cases to
            // this, but my apprach here should be good for our usecases.
        ...
        document.body.setAttribute('data-en-lang', lang);
    };
...
Internal.Template = function() {
     ...
     * @param {boolean} isNotification whether this is is an on-screen
     *                  notification. Important for correct click handlers.
     * @return {DocumentFragment}
     */
    me.makeAttachment = function(attachment, isNotification) {
        /** @type {DocumentFragment} */
        ...
        mainDiv.onclick = function() {
            if (isNotification) {
                Internal.Server.openNotificationAttachment(attachment.id,
                                                           attachment.type,
                                                           mainDiv);
            } else {
    ...
    var sanitizeHyperlink = function(originalHref) {
        ...
        // remove trailing characters typically used for punctuation
     ...
     * @param {Array<Internal.NotificationUser>} notificationUsers
     */
    me.transformAndAppendContent = function(node, textContent, notificationUsers) {
        /** @type {!Element} */
        ...
        // Transform notification user replacement tags
        if (notificationUsers) {
            for (var i = 0; i < notificationUsers.length; i++) {
                var notifyUser = notificationUsers[i];
                if (/^[a-z0-9]+$/i.test(notifyUser.templateID) && notifyUser.name) {
                    transform(new RegExp('<' + notifyUser.templateID + '>', 'g'), function() {
                        var element = document.createElement('b');
                        element.textContent = notifyUser.name;
                        return element;
                    });
                }
            }
    ...
    me.makeMessage = function(message) {
        ...
        message.isNotification = message.isNotification || false;
        message.notificationUsers = message.notificationUsers || [];
        ...
        if (message.groupingDisabled) {
            main.setAttribute('data-message-grouping-disabled', '1');
        }
        if (message.isSelf) {
            main.setAttribute('class', 'message right');
        }
        if (message.isNotification) {
            main.setAttribute('class', 'message notification');
        }
        ...
        me.transformAndAppendContent(content, message.body, message.notificationUsers);
...
Internal.View = function() {
    ...
    /**
     * Returns the grouping disabled attribute.
     *
     * @param {!(Node|DocumentFragment)} message
     * @return {boolean}
     */
    var getGroupingDisabled = function(message) {
        ...
        var groupingDisabled = messageField.getAttribute('data-message-grouping-disabled');

        if (groupingDisabled == null || groupingDisabled === '') {
            return false;
        }

        return groupingDisabled === '1';
    };
    ...
    me.addMessage = function(message) {
        ...
        var myGroupingDisabled = getGroupingDisabled(message);

        // Special case according to spec where we just always put it at bottom
        if (myTimestamp === -1 || myGroupingDisabled) {
