...
function MUI (){									
	...
	var start = "MUUIIUU", fin = "MIIIIIIII"; // ЮЗЕР   ??? если какой-нибудь эл-т нулевой(какой id сделать?)
	...
	for (var i = 0; i < infoMaxMin["maxNumAll"]; i++) { //  перебор всех строк
		...
		document.getElementById(arrRez[i][11]).onclick = function illuminatClick(e) {
			...
			for (var j=0; j < arrSaveClk.length; j++) {  
				arrSaveClk[j].style.backgroundColor = colorOut;	
			};
			arrSaveClk = [];
			for (var j=0; j < arrSaveClkDn.length; j++) {  
				arrSaveClkDn[j].style.backgroundColor = colorOut;	
			};
			arrSaveClkDn = [];
			if (saveClk) saveClk.style.backgroundColor = colorOut;
			saveClk = "";
			...
			while (row){
				arrSaveClk.push(row);
				row.style.backgroundColor = colorPreClick;
			...
			for (var j=0; j < arrTmpClassAll.length; j++) {
				...
				for (var k=0; k < arrTmpClass.length; k++) {
					...
					arrTmpClass[k].style.backgroundColor = colorPostClick;   
			...
			this.style.backgroundColor = colorThis;
			saveClk = this;
		};
		...
		document.getElementById(arrRez[i][11]).onmouseover = function illuminatOver(e) {
			if (~arrSaveClk.indexOf(this)) {return};// игнор постоянной подсветки
			
			///  окраска при наведении (вверх) ///
			var row = this;

			while (row){
				row.style.backgroundColor = colorPreOver;
				row = document.getElementById(row.className);
			};
			...
			for (var j=0; j < arrTmpClassAll.length; j++) {
				...
				var arrTmpClass = document.getElementsByClassName(tmpID);
				for (var k=0; k < arrTmpClass.length; k++) {
					arrTmpClass[k].style.backgroundColor = colorPostOver;  
					arrTmpClassAll.push(arrTmpClass[k].id);
				}
				arrTmpClassAll.splice(0,1);
				j--;
		...
		document.getElementById(arrRez[i][11]).onmouseout = function illuminatOut(e) {
			...
			var row = this;
			while (row){
				row.style.backgroundColor = colorOut;
				row = document.getElementById(row.className);
			};
			///  сброс окраски вниз  ///
			var rowDown = this;
			...
			for (var j=0; j < arrTmpClassAll.length; j++) {
				...
				var arrTmpClass = document.getElementsByClassName(tmpID);
				for (var k=0; k < arrTmpClass.length; k++) {
					arrTmpClass[k].style.backgroundColor = colorOut;  
					arrTmpClassAll.push(arrTmpClass[k].id);
				}
				arrTmpClassAll.splice(0,1);
				j--;
			...
			for (var j=0; j < arrSaveClk.length; j++) {
				arrSaveClk[j].style.backgroundColor = colorPreClick;	
			};
			for (var j=0; j < arrSaveClkDn.length; j++) {
				arrSaveClkDn[j].style.backgroundColor = colorPostClick;	   
			};
			saveClk.style.backgroundColor = colorThis;
		};
	}	
}
