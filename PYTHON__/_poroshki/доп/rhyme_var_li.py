...
#################### ПАРАМЕТРЫ ПРОГРАММЫ ###########################
####################################################################  
MODE=1  # ВЫБОР СТРОГОСТИ РИФМ  1 - обычный, 2 - упрощённый 

EASY=0  # ВЫБОР ГЛУБИНЫ РИФМ  0 - лёгкий (ударный слог), 1 - средний (с гласной в предударном слоге),   иначе (н/р 2) - тяжелый (с предударным слогом)
#################################################################### 



"""  ??       ВАРИАНТ I (усложнённый)
...
"""           ВАРИАНТ II (упрощённый)
Определить другие замены, выпадания ...     ??     

Определить другие родственные буквы     ??   
"""

MAP_2   = {      # карта 2  для безударных  (дополнительная к 2  не самостоятельная добивка, упрощение) 
...
MAP_COMMON = MAP_1 if MODE == 1 else (MAP_1 | MAP_2)  # для ударных  1 строгая  2 упрощённая 
MAP_NO_ACC = MAP_1 | MAP_3 | MAP_2     # для безударных



# мой фономизатор по моему словарю REPLACES     
def fix_with_dict(line: str, replaces=None) -> str:
    if replaces is None: replaces = MAP_COMMON
...
def two_rhymes_fn(a_tail, b_tail, easy) -> bool:
    ...
    for i in range(ln):
        ...
        if i == 0 or i >= 2:   # ??? разделить i == 0, i = 2 и управлять отдельно?    для первого слога  и  послеударного хвоста  упрощённая карта  
            ...
            a = remove_doubles_cons(fix_with_dict(a, MAP_NO_ACC))  # повторно remove_doubles_cons (надо)   
            b = remove_doubles_cons(fix_with_dict(b, MAP_NO_ACC))  
...
как мир огромный круглый сёгун 
